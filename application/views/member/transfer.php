<?php $usr_id = $this->session->userdata('member_login_id');?>
			...
			<div class="row">
				<div class="col-xs-8 offset-xs-2 col-sm-8 offset-sm-2 col-md-6 offset-md-3 col-lg-5 offset-lg-3">
					<div class="card rounded">
						...
						<div class="card-body">
							<form method="post" action="<?php echo base_url().'index.php?member/do_trans/'.$usr_id;?>" id="transferfund" class="mb-2">
								<div class="row">
	...
	<script> 
		...
		$('#send').click(function(){
			...
			if( rec != '' && amt != '')
			{
				$.ajax(
				{
					url: "<?php echo base_url().'index.php?member/chk_trans/'.$usr_id;?>",
					type: 'POST',
		...
		function submit()
		{
			$.ajax(
			{
				url: "<?php echo base_url().'index.php?member/do_trans/'.$usr_id;?>",
				type: 'POST',
				data: $('#transferfund').serialize(),
				cache: false,
				success: function(data)
				{
					...
					if(val.type == 'success')
					{
						swal({
							...
							window.location = "<?php echo base_url().'index.php?member/transfer';?>";
						});
					}
				}
			});
